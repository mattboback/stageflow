...
import { MAX_SSE_PARSE_ERRORS, MAX_SSE_RECONNECT_ATTEMPTS } from '$lib/stores/scan-status/constants';
...
export function createSSEStream<TStatus, TUpdate>(
	...
	const handleParseError = (eventType: string, err: unknown, rawData: string) => {
		...
		if (sseParseErrors >= MAX_SSE_PARSE_ERRORS) {
			...
			callbacks.onError({ message: 'Too many parse errors', parseError: true, terminal: true });
			close();
		}
	};
	...
	eventSource.onerror = (event) => {
		if (eventSource?.readyState === EventSource.CLOSED) {
			console.warn(`[${sourceName}] SSE connection closed permanently`, { jobId });
			options?.onLog?.(
				sourceName === 'scan-status'
					? 'WARN: Connection closed. Fetching latest status...'
					: 'WARN: Connection closed. Switching to polling...'
			);
			callbacks.onError({ message: 'Connection closed permanently', terminal: true });
			close();
			return;
		}
		...
		if (sseReconnectAttempts >= MAX_SSE_RECONNECT_ATTEMPTS) {
			...
			callbacks.onError({ message: 'Max reconnect attempts reached', terminal: true });
			close();
			return;
		}
