<script lang="ts">
	...
	const { jobId, status, report, job, logs, screenshots, error, onRefreshArtifacts }: Props =
		$props();
...
<PageSection class="pt-16" containerClass="max-w-7xl xl:max-w-[1400px]">
	...
	{#if displayReport}
		...
		<svelte:boundary onerror={(e) => console.error('Report section render error:', e)}>
			...
			{:else if section === 'artifacts'}
				<ArtifactsView {jobId} {job} {onRefreshArtifacts} />
	...
	{#if displayReport && selectedIssue}
		<IssueDetailModal
			...
			{audience}
			{screenshots}
			highlightedElementId={activeElementId ?? undefined}
