...
func (s *Store) HandleJobCreated(ctx context.Context, payload *events.JobCreatedPayload) error {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO job_status (job_id, state, input_type, created_at, updated_at, total_pages, current_page)
		VALUES (?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(job_id) DO NOTHING
	`, payload.JobID, state, payload.InputType, now, now, totalPages, currentPage)
...
// HandleScanCompleted advances state to COMPLETING and records totals.
func (s *Store) HandleScanCompleted(ctx context.Context, payload *events.ScanCompletedPayload) error {
	...
	if _, err := s.db.ExecContext(ctx, `
		...
		SET total_pages = CASE
			...
			updated_at = ?
		WHERE job_id = ?
	`, payload.TotalPagesScanned, payload.TotalPagesScanned, payload.TotalPagesScanned, payload.TotalPagesScanned, payload.Summary.TotalViolations, now, payload.JobID); err != nil {
		return err
	}

	return s.advanceState(ctx, payload.JobID, models.JobStateCompleting, now)
}
