...
func TestCollectScreenshotRefs(t *testing.T) {
	...
	if got := refs["a1"][0].ScannerType; got != "axe" {
...
func TestResolveOverviewScannerV2(t *testing.T) {
	...
	issues := []report.IssueDetail{
		...
		{Scanner: "axe", PageId: "page-1"},
		{Scanner: "custom", PageId: "page-2"},
	}

	if got, want := resolveOverviewScannerV2(issues, "page-1"), "axe"; got != want {
	...
	if got, want := resolveOverviewScannerV2(issues, "page-3"), "axe"; got != want {
