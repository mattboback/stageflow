...
describe("loadAxeScreenshotConfig", () => {
  ...
  it("uses sensible defaults", () => {
    delete process.env.A11Y_HIGHLIGHT_STYLE;
    delete process.env.A11Y_SCREENSHOT_FORMAT;
    delete process.env.A11Y_OVERLAY_METHOD;
    delete process.env.A11Y_SEMANTIC_OVERLAY_ENABLED;
    delete process.env.A11Y_SEMANTIC_OVERLAY_MAX_HEADINGS;
    delete process.env.A11Y_SEMANTIC_OVERLAY_MAX_LABEL;
    delete process.env.A11Y_SEMANTIC_OVERLAY_LEGEND;
  ...
  it("applies overrides last", () => {
    const cfg = loadAxeScreenshotConfig({ outputFormat: "png", webpQuality: 10 });
    expect(cfg.outputFormat).toBe("png");
