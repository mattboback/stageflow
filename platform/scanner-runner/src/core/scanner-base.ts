...
export abstract class ScannerBase {
  ...
  private async iteratePages(
    ...
    const { value: pageResults, durationMs: pageIterationMs } = await this.runPhase(
      "pageIteration",
      () =>
        this.pageIterator.iteratePages(provenance, (context) => this.scanPage(context), {
          ...
          onPageError: hooks.onError
            ? async (error, pageEntry) => hooks.onError?.(error, { pageEntry })
            : undefined,
        }),
    );
  ...
  private async handleRunError(error: unknown): Promise<void> {
    ...
    if (this.scanStageLogger) {
      try {
        this.scanStageLogger.recordEvent("scan_failed", { error: errorMessage });
  ...
  protected async uploadArtifacts(): Promise<void> {
    ...
    try {
      ...
      // Screenshots are optional, don't fail if directory doesn't exist
    }
