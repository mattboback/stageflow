...
while IFS=: read -r file lineno text; do
	...
	grep -rn \
		...
		-E '(^|[^a-zA-Z0-9_/.-])(apps/|tools/[a-z]|frontend/|docs/CONFIGURATION\.md|just run frontend|Scan Worker \(|Release stageflow CLI|project-mode scan using \.stageflow/config\.yaml|SvelteKit|Storybook|prettier-plugin-svelte|clients/web/src/|\.svelte-kit)' \
		. \
	| grep -v 'bun\.lock' \
	| grep -v 'CHANGELOG\.md' \
	| grep -v 'devtools/scripts/check-stale-vocab\.sh' \
