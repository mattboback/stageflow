...
- The axe scanner hung forever on a page containing an `<iframe sandbox>` without `allow-scripts` (for example a `srcdoc` document preview): axe-core cannot be injected into such a frame and the attempt never returned, so the job sat in "scanning" until the deadline and lost every scanner's results. Those frames are now excluded from analysis.
- The SEO, Open Graph, spelling and link scanners read the page as soon as its HTML arrived, so a client-rendered app was still an empty shell: an authenticated 17-page scan reported "0 words" and a missing `<h1>` on 16 pages and checked no links. They now wait for network idle (capped at 5 seconds), as axe already did.
...
- Re-recorded the README CLI demo GIF (`docs/images/demo.gif`) with
  `--format json` so it actually demonstrates the machine-readable output
  (previously it ran the default human/text format), with idle time
  compressed and playback sped up so it stays brisk despite the larger
  terminal
...
- README: the bounding-box GIF now leads at the top of the page; the CLI
  demo GIF moved down into "Try It in 60 Seconds" next to the JSON example
  it demonstrates
