...
describe("PlaygroundPage", () => {
	...
	it("renders the scan configuration surface and handles empty scanners", async () => {
		...
		expect(
			await screen.findByText("No scanners available"),
		).toBeInTheDocument();
	...
	it("uses multiline placeholder and removes auto-https helper text", async () => {
		mockFetchScanners.mockResolvedValue({
			scanners: [
				createScanner("axe"),
				createScanner("lighthouse"),
				createScanner("ai-navigator"),
			],
			categories: [],
		});
		...
		const textarea = expectTextarea(
			await screen.findByLabelText("URLs to Scan"),
		);
		expect(textarea.placeholder).toBe(
			"example.com\nexample.com/about\nhttps://example.com/contact",
		);
		expect(
			screen.queryByText(/Enter one URL per line/i),
	...
	it("defaults to coverage preset and enables multiple scanners", async () => {
		...
		expect(screen.getByRole("button", { name: /axe/i })).toHaveAttribute(
			"aria-pressed",
			"true",
		);
		...
		expect(
			screen.getByRole("button", { name: /ai navigator/i }),
