...
const normalizeLine = (line: string) => line.trim().replace(/:$/, '').toLowerCase();
...
export const extractPrimaryFailureDetail = (summary?: string | null): string | null => {
	const details = extractFailureDetails(summary);
	return details[0] ?? null;
};

export const parseHeadingOrderContext = (detail?: string | null): HeadingOrderContext | null => {
...
export const formatHeadingOrderFlow = (context: HeadingOrderContext): string => {
