/**
 * WCAG color-contrast math for the in-report "Verify contrast" tool.
 *
 * Formulas follow WCAG 2.x: https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum
 */
...
/**
 * Parse a CSS color string (#rgb/#rgba/#rrggbb/#rrggbbaa, rgb(), rgba()) into RGBA.
...
/** Convenience: parse two color strings and compute their ratio; null if either is unparseable. */
...
/** Format a ratio for display, e.g. 4.6182 -> "4.62". */
export function formatRatio(ratio: number): string {
	return (Math.round(ratio * 100) / 100).toFixed(2);
}

/**
 * WCAG "large text": at least 18pt (24px), or at least 14pt (~18.67px) when bold.
 */
...
/**
 * Parse axe's fontSize strings, e.g. "10.0pt (13.3333px)", "13.3px", "12pt".
...
/** The conformance level a rule tests: color-contrast-enhanced is the AAA rule. */
...
/**
 * Human explanations for axe's color-contrast "incomplete" message keys —
 * the reason automatic verification failed.
 */
