	...
	switch (ruleId) {
		...
		case 'valid-lang':
			return {
				...
				before: '<html>',
				after: '<html lang="en">',
				notes: 'Use a valid BCP 47 language tag that matches the primary language of the page.'
			};
		case 'label':
			return {
				...
				notes: 'Prefer visible labels; use aria-label only when a visible label is not possible.'
			};
		case 'link-name':
			return {
				...
				notes: 'Prefer visible link text; fall back to aria-label when the link is icon-only.'
			};
		case 'button-name':
			return {
		...
		case 'focus-visible':
			return {
				...
				after: 'button:focus-visible { outline: 2px solid #0ea5e9; outline-offset: 2px; }',
				notes: 'Use :focus-visible to avoid distracting focus rings for mouse users.'
			};
		default:
			return null;
	}
