...
export function createSSEStream<TStatus, TUpdate>(
	...
	const handleParseError = (
		...
		if (sseParseErrors >= MAX_SSE_PARSE_ERRORS) {
			console.warn(
				`[${sourceName}] Too many SSE parse errors, stopping stream`,
			);
			options?.onLog?.(
	...
	eventSource.onerror = (event) => {
		...
		if (sseReconnectAttempts >= MAX_SSE_RECONNECT_ATTEMPTS) {
			console.warn(
				`[${sourceName}] SSE connection failed after max reconnect attempts`,
				{
					jobId,
		...
		if (!reportedSSEError) {
			...
			console.warn(
				`[${sourceName}] SSE connection error, waiting for reconnect`,
				{
					jobId,
			...
			options?.onLog?.(
				"WARN: Lost connection to live updates. Reconnecting...",
			);
