...
export const ToggleModes: Story = {
	play: async ({ args, canvasElement }) => {
		...
		await expect(canvas.getByTestId('selected-mode')).toHaveTextContent('Selected mode: zip');

		await userEvent.click(urlButton);
		await expect(args.onModeChange).toHaveBeenCalledWith('url');
		await expect(canvas.getByTestId('selected-mode')).toHaveTextContent('Selected mode: url');
	}
};
