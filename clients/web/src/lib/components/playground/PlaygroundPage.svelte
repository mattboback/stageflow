...
const hasValidInput = $derived(
	mode === "url" ? urls.trim().length > 0 : file !== null,
);
const hasEnabledScanner = $derived(scanners.some((s) => s.enabled));
const isAiNavigatorEnabled = $derived(
	scanners.some((s) => s.id === "ai-navigator" && s.enabled),
);
const isAiConfigValid = $derived(
	!isAiNavigatorEnabled ||
		(aiObjective.trim().length > 0 && aiModel.trim().length > 0),
);
