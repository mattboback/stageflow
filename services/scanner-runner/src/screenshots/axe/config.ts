...
function parseOverlayMethod(raw: string): OverlayMethod {
	return raw.toLowerCase() === "css-injection"
		? "css-injection"
		: "sharp-composite";
}
...
export function loadAxeScreenshotConfig(
	...
	const outputFormat = parseOutputFormat(
		getEnvString("A11Y_SCREENSHOT_FORMAT", "webp"),
	);
	...
	return {
		...
		contextRatio: Math.max(
			0,
			getEnvNumber("A11Y_SCREENSHOT_CONTEXT_RATIO", 1.0),
		),
		...
		webpQuality: Math.min(
			100,
			Math.max(1, getEnvInt("A11Y_SCREENSHOT_QUALITY", 80)),
		),
		overlayMethod,
		...
		semanticOverlayLegendEnabled: getEnvBool(
			"A11Y_SEMANTIC_OVERLAY_LEGEND",
			true,
		),
		...overrides,
	};
