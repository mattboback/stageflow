import type { AxeScreenshotConfig, HighlightStyle, OutputFormat, OverlayMethod } from './types';
...
export function loadAxeScreenshotConfig(
	...
	const highlightStyle = parseHighlightStyle(getEnvString('A11Y_HIGHLIGHT_STYLE', 'solid'));
	const outputFormat = parseOutputFormat(getEnvString('A11Y_SCREENSHOT_FORMAT', 'webp'));
	const overlayMethod = parseOverlayMethod(getEnvString('A11Y_OVERLAY_METHOD', 'sharp-composite'));
	...
	return {
		...
		elementContextMultiplier: Math.max(1, getEnvNumber('A11Y_ELEMENT_CONTEXT_MULTIPLIER', 2.5)),
		elementContextMinSize: Math.max(0, getEnvNumber('A11Y_ELEMENT_CONTEXT_MIN_SIZE', 400)),
		...
		semanticOverlayMaxHeadings: Math.max(0, getEnvInt('A11Y_SEMANTIC_OVERLAY_MAX_HEADINGS', 30)),
		semanticOverlayMaxLabelLength: Math.max(0, getEnvInt('A11Y_SEMANTIC_OVERLAY_MAX_LABEL', 60)),
