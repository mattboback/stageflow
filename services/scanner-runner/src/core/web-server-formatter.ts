...
export class WebServerFormatter {
	format(
		...
		const baseUrl =
			typeof provenance.base_url === "string" &&
			provenance.base_url.trim().length > 0
				? provenance.base_url
				: undefined;
		...
		for (const page of results.pages) {
			...
			if (rawResults?.pageOverview?.screenshotFilename) {
				artifacts.push({
					...
					mime: this.guessScreenshotMime(
						rawResults.pageOverview.screenshotFilename,
					),
				});
			}
			...
			for (const issue of page.issues) {
				...
				const nodes = Array.isArray(issueMetadata.nodes)
					? issueMetadata.nodes
					: [];
				const nodeCount =
				...
				const occurrences: NonNullable<
					UnifiedReportV2["issues"][number]["occurrences"]
				...
				const severityRaw =
					typeof issueMetadata.impact === "string"
						? issueMetadata.impact
						: undefined;
