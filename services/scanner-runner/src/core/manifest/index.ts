...
import type {
	...
	ScannerManifest,
} from "@stageflow/contracts-scanner-manifest";
...
export function validateManifest(
	...
	if (!manifest.description) {
		warnings.push({
			...
			suggestion:
				"Add a description to help users understand the scanner purpose",
		});
	}
	...
	if (manifest.configSchema !== undefined) {
		if (
			...
			typeof manifest.configSchema !== "boolean"
		) {
			errors.push({
				path: "/configSchema",
