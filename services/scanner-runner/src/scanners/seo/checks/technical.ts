...
export const TECHNICAL_CHECKS: SEOCheck[] = [
	{
		...
		check: (data) => {
			if (!data.canonical) {
				return {
					passed: false,
					message: 'Page is missing a canonical URL. This helps prevent duplicate content issues.'
				};
			}
	...
	},
	{
		...
		check: (data) => {
			if (!data.viewport) {
				return {
					passed: false,
					message: 'Page is missing viewport meta tag. This affects mobile usability and SEO.'
				};
			}
