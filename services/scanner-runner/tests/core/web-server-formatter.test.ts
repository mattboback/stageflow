...
describe("WebServerFormatter", () => {
	it("formats scan results into the v2 web-server schema", () => {
		...
		expect(formatted.pages[0]!.pageOverview?.screenshotFilename).toBe(
			"overview.png",
		);
		...
		const overviewArtifact = artifacts.find(
			(entry) => entry.type === "page-overview",
		);
		expect(overviewArtifact).toBeDefined();
		expect(overviewArtifact?.path).toBe("screenshots/overview.png");

		const formatted2 = new WebServerFormatter().format(
			provenance,
			results,
			metadata,
		);
	...
	it("sets screenshot mime based on file extension", () => {
		...
		const metadata: ScannerMetadata = { name: "axe", version: "0.1.0" };
		const formatted = new WebServerFormatter().format(
			provenance,
			results,
			metadata,
		);
