...
function baseManifest(
	overrides: Partial<ScannerManifest> = {},
): ScannerManifest {
	return {
		id: "test-plugin",
		name: "Test Plugin",
...
function config(
	overrides: Partial<PluginLoaderConfig> = {},
): PluginLoaderConfig {
	return {
		searchPaths: [],
...
describe("loadPluginFromManifest", () => {
	...
	it("returns an error when entry point does not exist", async () => {
		tempRoot = await mkdtemp(
			path.join(os.tmpdir(), "stageflow-plugin-load-missing-"),
		);
	...
	it("returns an error when configured factory function is missing", async () => {
		tempRoot = await mkdtemp(
			path.join(os.tmpdir(), "stageflow-plugin-load-factory-missing-"),
		);
		const manifestPath = path.join(tempRoot, "manifest.json");
	...
	it("returns an error when factory output does not implement scanPage", async () => {
		tempRoot = await mkdtemp(
			path.join(os.tmpdir(), "stageflow-plugin-load-invalid-factory-"),
		);
		const manifestPath = path.join(tempRoot, "manifest.json");
		const modulePath = path.join(tempRoot, "scanner.mjs");
		...
		expect(result.error).toBe(
			"Scanner does not implement required interface (scanPage method)",
		);
	});

	it("returns an instantiation error when factory throws", async () => {
		tempRoot = await mkdtemp(
			path.join(os.tmpdir(), "stageflow-plugin-load-factory-throws-"),
		);
		const manifestPath = path.join(tempRoot, "manifest.json");
		const modulePath = path.join(tempRoot, "scanner.mjs");
	...
	it("returns an error when exportName is missing from module exports", async () => {
		tempRoot = await mkdtemp(
			path.join(os.tmpdir(), "stageflow-plugin-load-export-missing-"),
		);
		const manifestPath = path.join(tempRoot, "manifest.json");
		const modulePath = path.join(tempRoot, "scanner.mjs");
	...
	it("returns an error when export is not a constructor", async () => {
		...
		await writeModule(
			modulePath,
			["export const NotAClass = { nope: true };", ""].join("\n"),
		);
	...
	it("loads a plugin from default export and logs when verbose mode is enabled", async () => {
		tempRoot = await mkdtemp(
			path.join(os.tmpdir(), "stageflow-plugin-load-default-success-"),
		);
		const manifestPath = path.join(tempRoot, "manifest.json");
		const modulePath = path.join(tempRoot, "scanner.mjs");
	...
	it("returns import error details when module parsing fails", async () => {
		tempRoot = await mkdtemp(
			path.join(os.tmpdir(), "stageflow-plugin-load-import-error-"),
		);
		const manifestPath = path.join(tempRoot, "manifest.json");
		const modulePath = path.join(tempRoot, "scanner.mjs");
