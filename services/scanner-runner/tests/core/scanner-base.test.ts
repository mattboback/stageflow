...
vi.mock("../../src/core/browser-manager", async () => {
	const actual = await vi.importActual<
		typeof import("../../src/core/browser-manager")
...
vi.mock("../../src/core/page-iterator", async () => {
	const actual = await vi.importActual<
		typeof import("../../src/core/page-iterator")
...
vi.mock("../../src/core/storage-provider", async () => {
	const actual = await vi.importActual<
		typeof import("../../src/core/storage-provider")
...
vi.mock("../../src/core/event-publisher", async () => {
	const actual = await vi.importActual<
		typeof import("../../src/core/event-publisher")
	>("../../src/core/event-publisher");
	return {
		...actual,
		NatsEventPublisher: vi
			.fn()
			.mockImplementation(function NatsPublisherMock() {
...
describe("ScannerBase", () => {
	...
	it("should initialize and run a scan successfully", async () => {
		...
		expect(
			mocks.mockEventPublisherInstance.publishScanCompleted,
		).toHaveBeenCalled();
		expect(
			mocks.mockEventPublisherInstance.publishPageCompleted,
	...
	it("should handle scan errors gracefully", async () => {
		...
		expect(
			mocks.mockEventPublisherInstance.publishScanFailed,
		).toHaveBeenCalledWith(
			"Provenance missing",
			expect.anything(),
			expect.objectContaining({
				stageLogPath: expect.stringContaining(
					"stages/scan.mock-scanner.log.json",
				),
	...
	it("uploads scanner artifacts when scan succeeds", async () => {
		...
		expect(
			mocks.mockBrowserManagerInstance.createContext,
	...
	it("validates provenance required fields and URL constraints", () => {
		...
		expect(() => {
			scanner.validateProvenanceInput({
				...
				base_url: "",
				pages: [
					{ id: "page-1", path: "/index.html", url: "https://example.com" },
				],
			} as unknown as Provenance);
		}).not.toThrow();
	});
	...
	it("uploads screenshot subdirectories for discovered page folders", async () => {
		...
		expect(
			mocks.mockStorageProviderInstance.uploadDirectory,
		).toHaveBeenCalledWith(
	...
	it("handles mixed extra artifact manifest entries", async () => {
		...
		mocks.mockStorageProviderInstance.upload.mockImplementation(
