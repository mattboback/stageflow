...
describe("ScreenshotService (integration)", () => {
  ...
  it(
    "captureWithHighlights produces a real outline near the element bounds and cleans up styles",
    async () => {
  ...
  it(
    "captureWithHighlights produces an outline at DPR=2 and returns scaled image dimensions",
    async () => {
      const context = await browser.newContext({
        viewport: { width: 400, height: 300 },
        deviceScaleFactor: 2,
