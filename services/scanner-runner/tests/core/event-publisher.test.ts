...
describe("NatsEventPublisher", () => {
	...
	it("throws for non-suppressed publishes when not connected", async () => {
		...
		await expect(
			publisher.publishPageCompleted(pageResult, 0, 1),
	...
	it("connects, publishes envelopes (with correlation ids), and closes cleanly", async () => {
		...
		const pageEnv = published.find(
			(m) => m.env.event === "scan.page.completed",
		)!.env;
		...
		const completedEnv = published.find(
			(m) => m.env.event === "scan.completed",
		)!.env;
