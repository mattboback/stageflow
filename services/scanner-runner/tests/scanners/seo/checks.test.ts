...
function baseData(overrides: Partial<PageSEOData> = {}): PageSEOData {
	return {
		...
		images: [
			{ src: "https://example.com/a.png", alt: "Alt", width: 10, height: 10 },
		],
		links: [
			{
				href: "/internal",
...
describe("SEO checks", () => {
	...
	describe("meta checks", () => {
		...
		it("flags title too short", () => {
			const result = getCheck("title-length").check(
				baseData({ title: "Short" }),
			);
			expect(result).not.toBeNull();
			expect(result?.passed).toBe(false);
	...
	describe("technical checks", () => {
		it("flags missing canonical URL", () => {
			const result = getCheck("missing-canonical").check(
				baseData({ canonical: null }),
			);
			expect(result).not.toBeNull();
			expect(result?.passed).toBe(false);
		});
		...
		it("flags missing viewport meta tag", () => {
			const result = getCheck("missing-viewport").check(
				baseData({ viewport: null }),
			);
			expect(result).not.toBeNull();
			expect(result?.passed).toBe(false);
		});
		...
		it("flags missing language attribute", () => {
			const result = getCheck("missing-language").check(
				baseData({ language: null }),
			);
			expect(result).not.toBeNull();
			expect(result?.passed).toBe(false);
		});
	...
	describe("content checks", () => {
		it("flags thin content", () => {
			const result = getCheck("thin-content").check(
				baseData({ wordCount: 42 }),
			);
			expect(result).not.toBeNull();
			expect(result?.passed).toBe(false);
		});
