...
describe('SEO checks', () => {
	...
	describe('meta checks', () => {
		...
		it('flags missing meta description', () => {
			const result = getCheck('missing-description').check(baseData({ description: null }));
			expect(result).not.toBeNull();
			expect(result?.passed).toBe(false);
		});

		it('flags description too short', () => {
			const result = getCheck('description-length').check(baseData({ description: 'Too short' }));
			expect(result).not.toBeNull();
			expect(result?.passed).toBe(false);
		});
	...
	describe('technical checks', () => {
		...
		it('flags missing structured data', () => {
			const result = getCheck('missing-structured-data').check(baseData({ structuredData: [] }));
			expect(result).not.toBeNull();
			expect(result?.passed).toBe(false);
		});
