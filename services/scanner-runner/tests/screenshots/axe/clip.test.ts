...
describe("computeUnionClip", () => {
  ...
  it("computes a padded clip and relative element bounds", () => {
    ...
    expect(result!.clip).toEqual({ x: 0, y: 5, width: 110, height: 70 });
    expect(result!.elementBounds).toEqual([
...
describe("elementBounds accuracy regression tests", () => {
  it("element bounds are positioned relative to clip origin", () => {
    ...
    const expectedRelativeX = boxes[0]!.box.x - result!.clip.x;
    const expectedRelativeY = boxes[0]!.box.y - result!.clip.y;

    expect(result!.elementBounds[0]!.x).toBe(expectedRelativeX);
    expect(result!.elementBounds[0]!.y).toBe(expectedRelativeY);
    expect(result!.elementBounds[0]!.width).toBe(boxes[0]!.box.width);
    expect(result!.elementBounds[0]!.height).toBe(boxes[0]!.box.height);
  });
  ...
  it("multiple elements maintain correct relative positions", () => {
    ...
    const elem1 = result!.elementBounds[0]!;
    const elem2 = result!.elementBounds[1]!;
  ...
  it("element bounds x/y should never be negative within screenshot", () => {
    ...
    const viewport = { width: 1280, height: 720 };
    const boxes = [{ selector: "#edge", box: { x: 10, y: 10, width: 100, height: 50 } }];

    const result = computeUnionClip(boxes, viewport, cfg);
    expect(result).not.toBeNull();

    // Element bounds should be non-negative (within the screenshot)
    expect(result!.elementBounds[0]!.x).toBeGreaterThanOrEqual(0);
    expect(result!.elementBounds[0]!.y).toBeGreaterThanOrEqual(0);
  });
  ...
  it("element should be fully contained within the clip region", () => {
    ...
    const viewport = { width: 1280, height: 720 };
    const boxes = [
      { selector: "#contained", box: { x: 300, y: 200, width: 150, height: 80 } },
    ];

    const result = computeUnionClip(boxes, viewport, cfg);
    expect(result).not.toBeNull();

    const elem = result!.elementBounds[0]!;
