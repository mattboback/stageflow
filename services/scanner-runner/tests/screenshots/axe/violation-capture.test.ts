...
const createConfig = (overrides: Partial<AxeScreenshotConfig> = {}): AxeScreenshotConfig => ({
	...DEFAULT_CONFIG,
	...overrides,
	elementContextMultiplier:
		overrides.elementContextMultiplier ?? DEFAULT_CONFIG.elementContextMultiplier,
	elementContextMinSize: overrides.elementContextMinSize ?? DEFAULT_CONFIG.elementContextMinSize
});

// Helper to create mock locator
const createMockLocator = (boundingBox = { x: 100, y: 100, width: 50, height: 30 }): Locator => {
	const locator = {
...
describe('captureViolationScreenshot', () => {
	...
	describe('policy handling', () => {
		it('delegates to semantic overlay for semantic policy', async () => {
			...
			const { captureSemanticOverlayScreenshot } =
				await import('../../../src/screenshots/axe/semantic-overlay');

			(getScreenshotPolicy as ReturnType<typeof vi.fn>).mockReturnValueOnce('semantic');
		...
		it('continues with default capture for default policy', async () => {
			const { getScreenshotPolicy } = await import('../../../src/config/rule-behaviors');
			(getScreenshotPolicy as ReturnType<typeof vi.fn>).mockReturnValueOnce('default');
	...
	describe('screenshot strategies', () => {
		describe('union bounding box strategy', () => {
			it('uses union strategy when mergeTargets is true and multiple targets exist', async () => {
				const { computeUnionClip } = await import('../../../src/screenshots/axe/clip');
				const mockPage = createMockPage();
			...
			it('falls back when union clip returns null', async () => {
				const { computeUnionClip } = await import('../../../src/screenshots/axe/clip');
				(computeUnionClip as ReturnType<typeof vi.fn>).mockReturnValueOnce(null);
		...
		describe('single target viewport strategy', () => {
			it('tries single target when selector exists', async () => {
				const { computeUnionClip, computeSingleTargetClip } =
			...
			it('falls back when single target clip fails', async () => {
				const { computeUnionClip, computeSingleTargetClip, computeElementClip } =
					await import('../../../src/screenshots/axe/clip');
				(computeUnionClip as ReturnType<typeof vi.fn>).mockReturnValueOnce(null);
				(computeSingleTargetClip as ReturnType<typeof vi.fn>).mockReturnValueOnce(null);
		...
		describe('element screenshot strategy', () => {
			...
			it('falls back to viewport when element clip fails', async () => {
				...
				(computeUnionClip as ReturnType<typeof vi.fn>).mockReturnValueOnce(null);
				(computeSingleTargetClip as ReturnType<typeof vi.fn>).mockReturnValueOnce(null);
				(computeElementClip as ReturnType<typeof vi.fn>).mockReturnValueOnce(null);
		...
		describe('viewport fallback', () => {
			it('captures viewport screenshot as last resort', async () => {
				...
				(mockLocator.boundingBox as ReturnType<typeof vi.fn>).mockResolvedValue(null);
				const mockPage = createMockPage({
	...
	describe('CSS injection', () => {
		it('injects highlight CSS when using css-injection method', async () => {
			const { injectHighlightCSS } = await import('../../../src/screenshots/axe/highlight-css');
			const mockPage = createMockPage();
			const config = createConfig({
				overlayMethod: 'css-injection',
				injectHighlight: true
		...
		it('removes highlight CSS in finally block', async () => {
			const { removeHighlightCSS } = await import('../../../src/screenshots/axe/highlight-css');
			const mockPage = createMockPage();
		...
		it('applies sharp composite overlay when using sharp-composite method', async () => {
			const { compositeOverlay } = await import('../../../src/screenshots/axe/image');
			const mockPage = createMockPage();
			const config = createConfig({
