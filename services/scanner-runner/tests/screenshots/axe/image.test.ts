...
describe('compositeOverlay', () => {
	...
	it('returns original buffer when no element bounds', async () => {
		...
		const result = await compositeOverlay(buffer, [], {
	...
	it('draws overlay at CSS coordinates when no DPI scaling', async () => {
		...
		const buffer = await createTestImage(100, 100);
		const bounds: ElementBounds[] = [{ x: 10, y: 20, width: 30, height: 40, selector: '#test' }];

		const result = await compositeOverlay(buffer, bounds, { highlightStyle: 'solid' }, 100);
	...
	describe('DPI scaling', () => {
		it('scales element bounds by 2x when actual pixels are 2x CSS pixels', async () => {
			...
			const result = await compositeOverlay(buffer, bounds, { highlightStyle: 'solid' }, cssWidth);
		...
		it('defaults to DPR 1 when clipCssWidth not provided', async () => {
			...
			const result = await compositeOverlay(buffer, bounds, {
				highlightStyle: 'solid'
			});
		...
		it('defaults to DPR 1 when clipCssWidth is 0', async () => {
			...
			const result = await compositeOverlay(buffer, bounds, { highlightStyle: 'solid' }, 0);
	...
	describe('overlay positioning regression tests', () => {
		...
		it('stroke width scales with DPR for consistent visual appearance', async () => {
			...
			const result = await compositeOverlay(buffer, bounds, { highlightStyle: 'solid' }, 1280);
			expect(result).toBeDefined();
	...
	describe('pixel positioning assertions', () => {
		it('tints pixels inside bounds at DPR=1', async () => {
			const buffer = await createTestImage(200, 200);
			const bounds: ElementBounds[] = [{ x: 20, y: 30, width: 40, height: 50, selector: '#test' }];

			const result = await compositeOverlay(buffer, bounds, { highlightStyle: 'solid' }, 200);
	...
	describe('highlight styles', () => {
		it('supports solid highlight style', async () => {
			const buffer = await createTestImage(100, 100);
			const bounds: ElementBounds[] = [{ x: 10, y: 10, width: 20, height: 20, selector: '#test' }];
		...
		it('supports dashed highlight style', async () => {
			...
			const result = await compositeOverlay(buffer, bounds, { highlightStyle: 'dashed' }, 100);
			expect(result).toBeDefined();
		});
	});
});
