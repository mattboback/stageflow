...
const createMockPage = (): Page => {
	return {
		...
		locator: vi.fn().mockReturnValue({
			...
			boundingBox: vi
				.fn()
				.mockResolvedValue({ x: 100, y: 100, width: 50, height: 30 }),
			evaluate: vi.fn().mockResolvedValue({}),
		}),
...
describe("AxeScreenshotService", () => {
	...
	describe("captureViolationScreenshot", () => {
		it("delegates to captureViolationScreenshot function", async () => {
			const { captureViolationScreenshot: mockCapture } = await import(
				"../../src/screenshots/axe/violation-capture"
			);
			const service = new AxeScreenshotService();
			...
			const result = await service.captureViolationScreenshot(
				mockPage,
				violation,
				"/tmp/results",
			);
	...
	describe("capturePageOverview", () => {
		...
		it("returns null when capture fails", async () => {
			const { capturePageOverviewRaw } = await import(
				"../../src/screenshots/axe/page-overview"
			);
			(
				capturePageOverviewRaw as ReturnType<typeof vi.fn>
		...
		it("uses pageId in filename", async () => {
			...
			await service.capturePageOverview(
				mockPage,
				[],
				"/tmp/results",
				"unique-page-id",
	...
	describe("config propagation", () => {
		it("passes config to violation capture", async () => {
			...
			await service.captureViolationScreenshot(
				mockPage,
				{ id: "test" },
				"/tmp/results",
			);
