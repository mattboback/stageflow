...
const sourceManifestTypesPath = path.join(sourceTypesDir, "scanner-manifest.ts");
...
if (!(await pathExists(sourceIndexPath)) || !(await pathExists(sourceManifestTypesPath))) {
  throw new Error(
...
await fs.cp(sourceManifestTypesPath, path.join(packageRoot, "scanner-manifest.ts"));
